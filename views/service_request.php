...
function hasScheduleConflicts($db, $request_id) {
    $query = "SELECT date_of_travel, time_departure, time_return, vehicle_id, driver_emp_id 
    ...
    // Check for conflicts
    $conflicts = checkScheduleConflicts($db, $request['date_of_travel'], $request['time_departure'], 
                                      $request['time_return'], $request['vehicle_id'], 
                                      $request['driver_emp_id'], $passengers, $request_id, true);
...
function getRequestDetails($db, $request_id, $is_admin) {
    ...
    // Check if this request has conflicts
    ...
    <div style='text-align: left;'>
        ...
        " . ($has_conflicts ? "<div class='alert alert-warning'><i class='fas fa-exclamation-triangle'></i> This request has schedule conflicts!</div>" : "") . "
        ...
        <p><strong>Date of Travel:</strong> " . date('m/d/Y', strtotime($request['date_of_travel'])) . "</p>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['create_request'])) {
        try {
            ...
            $conflicts = checkScheduleConflicts($db, $date_of_travel, $time_departure, 
                                                $time_return, $vehicle_id, 
            ...
            $query = "INSERT INTO service_requests 
                    (request_no, requesting_emp_id, supervisor_emp_id, date_requested, date_of_travel, 
                      time_departure, time_return, destination, purpose, vehicle_id, driver_emp_id, status)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')";
            $current_date = date('Y-m-d');

            $stmt = $db->prepare($query);
            $stmt->bind_param("siissssssii", 
                $request_no,
                $requesting_emp_id,
        ...
        try {
            ...
            if ($current_status === 'approved') {
                $conflicts = checkScheduleConflicts($db, $date_of_travel, $time_departure, 
                                                    $time_return, $vehicle_id, 
                                                    $driver_emp_id, $passengers, $request_id);
            }
            ...
            // Update passengers
            $delete_passengers = "DELETE FROM service_request_passengers WHERE request_id = ?";
            $delete_stmt = $db->prepare($delete_passengers);
      ...
      try {
          ...
          // Fetch request details to check for conflicts
          $query = "SELECT date_of_travel, time_departure, time_return, vehicle_id, driver_emp_id 
                    FROM service_requests WHERE request_id = ?";
          $stmt = $db->prepare($query);
          $stmt->bind_param("i", $request_id);
...
function checkScheduleConflicts($db, $date, $time_start, $time_end, $vehicle_id, $driver_id, $passengers, $exclude_request_id = null, $check_for_approval = false) {
    $conflicts = [];
    error_log("Conflict check called with:");
    error_log("Date: $date, Time Start: $time_start, Time End: $time_end");
    ...
    try {
        ...
        $end_datetime = new DateTime("$date $time_end");
    } catch (Exception $e) {
    ...
    if ($end_datetime <= $start_datetime) {
        $conflicts[] = ['message' => "Invalid time range: Return time must be after departure time."];
        return $conflicts;
    }
    ...
    if ($travel_date < $current_date->setTime(0, 0, 0)) {
        $conflicts[] = ['message' => "Cannot schedule travel for a past date."];
    }

    // Check vehicle, driver, and passenger conflicts
    $vehicle_conflicts = checkVehicleConflicts($db, $vehicle_id, $date, $time_start, $time_end, $exclude_request_id);
    $conflicts = array_merge($conflicts, $vehicle_conflicts);

    $driver_conflicts = checkDriverConflicts($db, $driver_id, $date, $time_start, $time_end, $exclude_request_id);
    $conflicts = array_merge($conflicts, $driver_conflicts);

    $passenger_conflicts = checkPassengerConflicts($db, $passengers, $date, $time_start, $time_end, $exclude_request_id);
...
function checkVehicleConflicts($db, $vehicle_id, $date, $time_start, $time_end, $exclude_request_id = null) {
    $conflicts = [];
    ...
    $vehicle_query = "SELECT sr.request_id, sr.request_no, sr.date_of_travel, 
                 sr.time_departure, sr.time_return, v.model, v.plate_no,
                 CONCAT(req.first_name, ' ', req.last_name) AS requester_name,
                 sr.status
                 FROM service_requests sr
    ...
    if ($exclude_request_id) {
        $stmt->bind_param("isssssssssi", $vehicle_id, $date, $time_end, $time_start, 
                         $time_start, $time_end, $time_start, $time_end,
                         $time_start, $time_end, $exclude_request_id);
    } else {
        $stmt->bind_param("isssssssss", $vehicle_id, $date, $time_end, $time_start, 
                         $time_start, $time_end, $time_start, $time_end,
                         $time_start, $time_end);
    }
    ...
    foreach ($vehicle_conflicts as $conflict) {
        $conflicts[] = [
            ...
            'message' => "Vehicle {$conflict['model']} ({$conflict['plate_no']}) is already booked for Request #{$conflict['request_no']} " .
                         "by {$conflict['requester_name']} on {$conflict['date_of_travel']} from " .
...
function checkDriverConflicts($db, $driver_id, $date, $time_start, $time_end, $exclude_request_id = null) {
    $conflicts = [];
    ...
    $driver_query = "SELECT sr.request_id, sr.request_no, sr.date_of_travel, 
                sr.time_departure, sr.time_return, 
    ...
    if ($exclude_request_id) {
        $stmt->bind_param("isssssssssi", $driver_id, $date, $time_end, $time_start, 
                         $time_start, $time_end, $time_start, $time_end,
                         $time_start, $time_end, $exclude_request_id);
    } else {
        $stmt->bind_param("isssssssss", $driver_id, $date, $time_end, $time_start, 
                         $time_start, $time_end, $time_start, $time_end,
                         $time_start, $time_end);
    }
    ...
    foreach ($driver_conflicts as $conflict) {
        ...
        $conflicts[] = [
            ...
            'message' => "Driver {$conflict['driver_name']} is already assigned to Request #{$conflict['request_no']}" .
                         "{$vehicle_info} by {$conflict['requester_name']} on {$conflict['date_of_travel']} from " .
                         date('h:i A', strtotime($conflict['time_departure'])) . " to " .
                         date('h:i A', strtotime($conflict['time_return'])) . "."
        ];
    }
...
function checkPassengerConflicts($db, $passengers, $date, $time_start, $time_end, $exclude_request_id = null) {
    $conflicts = [];
    ...
    $params = array_merge($passengers, [$date, $time_end, $time_start, 
                                      $time_start, $time_end, $time_start, $time_end,
                                      $time_start, $time_end]);
...
$supervisors = $db->query("SELECT emp_id, CONCAT(first_name, ' ', last_name) AS full_name FROM employee WHERE is_manager = 1 ORDER BY last_name, first_name")->fetch_all(MYSQLI_ASSOC);
                ...
                <table id="requestsTable" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td><?= date('m/d/Y', strtotime($request['date_of_travel'])) ?></td>
                      ...
                      <td>
                        ...
                        // Add conflict status if applicable
            ...
            <div class="row">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control select22" multiple="multiple" name="supervisor_emp_id" required>
                    ...
                    <?php foreach ($supervisors as $supervisor): ?>
                      <option value="<?= $supervisor['emp_id'] ?>"><?= $supervisor['full_name'] ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <input type="date" class="form-control" name="date_of_travel" required min="<?= date('Y-m-d') ?>">
                </div>
              </div>
            </div>
          ...
          <div class="modal-body">
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label>Date of Travel</label>
                  <input type="date" class="form-control" name="date_of_travel" value="<?= $edit_request['date_of_travel'] ?>" required>
                </div>
              </div>
            </div>
                  ...
                  <select class="form-control select22" multiple="multiple" name="vehicle_id" required>
                    ...
                    <?php foreach ($vehicles as $vehicle): ?>
                      <option value="<?= $vehicle['vehicle_id'] ?>" <?= ($vehicle['vehicle_id'] == $edit_request['vehicle_id']) ? 'selected' : '' ?>>
                        <?= $vehicle['vehicle_type'] ?> - <?= $vehicle['property_no'] ?> (<?= $vehicle['plate_no'] ?>)
                      </option>
                    <?php endforeach; ?>
                  </select>
                </div>
            ...
            <div class="form-group">
              ...
              <select class="form-control select2" name="passengers[]" multiple="multiple" data-placeholder="Select passengers">
                <?php foreach ($employees as $employee): ?>
                  <?php if ($employee['emp_id'] != $_SESSION['emp_id']): ?>
                    <option value="<?= $employee['emp_id'] ?>" <?= in_array($employee['emp_id'], $edit_passengers) ? 'selected' : '' ?>>
                      <?= $employee['full_name'] ?>
                    </option>
                  <?php endif; ?>
                <?php endforeach; ?>
